...
# 3. centerMap
center_map_str = """  const centerMap = useCallback(() => {
...
animate_start_str = "  const animateLiveLocation = useCallback((newLoc: { lat: number; lng: number; heading?: number }) => {"
animate_end_str = "  }, [animateLiveLocation, animateMarker, dropCoord, initialDriverCoord, pickupCoord, subscribe, token, user]);"

animate_start_idx = code.find(animate_start_str)
animate_end_idx = code.find(animate_end_str, animate_start_idx)

if animate_start_idx != -1 and animate_end_idx != -1:
    new_animate = """  const animateLiveLocation = useCallback((newLoc: { lat: number; lng: number; heading?: number }) => {
...
"""
    # Replace the chunk
    # We replace from animate_start_idx up to animate_end_idx (not including it)
    code = code[:animate_start_idx] + new_animate + code[animate_end_idx:]
else:
                ...
                <View style={styles.driverMarkerContainer}>
                  ...
                  <View style={{ transform: [{ rotate: `${driverHeading}deg` }] }}>
                    <View style={styles.uberDriverMarker}>
                      {renderVehicleIcon(24, Colors.dark)}
                    </View>
                  </View>
                </View>
              </Marker>
...
code = code.replace("  driverTopRow: { flexDirection: \"row\"", styles_to_add + "  driverTopRow: { flexDirection: \"row\"")

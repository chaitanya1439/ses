...
export default function BookingConfirmedScreen() {
  ...
  const pickup = useMemo(() => ({
    lat: initPayload?.pickupLocation?.lat || 17.385,
    lng: initPayload?.pickupLocation?.lng || 78.4867,
    name: "Pickup",
    address: initPayload?.pickupAddress || "Pickup Location"
  }), [initPayload]);

  const drop = useMemo(() => ({
    lat: initPayload?.dropLocation?.lat || 17.4,
    lng: initPayload?.dropLocation?.lng || 78.5,
    name: "Drop",
    address: initPayload?.dropAddress || "Drop Location"
  }), [initPayload]);

  const selectedVehicle = initPayload?.vehicle || 'Auto';
  ...
  }, [isConfirmed, progressAnim, subscribe, selectedVehicle]);

...
import { useRiderMode } from "@/context/RiderModeContext";
...
export default function RideOptionsScreen() {
  ...
  const { pickup, drop, selectedVehicle, setSelectedVehicle, setFare, setRouteDetails } = useRiderMode();
  ...
  const doBookRide = () => {
    if (!routeDetailsLocal) return;
    const currentOption = dynamicVehicleOptions.find(v => v.id === selectedVehicle);
    if (!currentOption) return;

    // 1. Prepare ride data
    const rideData = {
      pickupLocation: pickupCoord,
      pickupAddress: pickup?.address || "Selected Location",
      dropAddress: drop?.address || "Selected Location",
      destinationLocation: dropCoord,
      vehicle: selectedVehicle,
      fare: currentOption.fare,
      distance: Number((routeDetailsLocal.distanceMeters / 1000).toFixed(1)),
      riderName: user?.name || "Driver (Rider Mode)",
    };

    // 2. Call the backend directly via socket
    sendThrottledMessage("ride_request", rideData);
    Alert.alert('Ride Requested!', `Searching for a nearby ${selectedVehicle} Captain...`, [
      { text: 'OK', onPress: () => router.replace('/(tabs)/home' as any) } // We go back to home, the global listener will catch ride_accepted
    ]);
  };

...
let authFile = '/media/callidus/callidus2/ses/Driver-Mobile-Assets/context/AuthContext.tsx';
...
// 2. Create app/registration-fee.tsx
let feeFile = '/media/callidus/callidus2/ses/Driver-Mobile-Assets/app/registration-fee.tsx';
let feeContent = `import React, { useState } from 'react';
...
`;
fs.writeFileSync(feeFile, feeContent);

// 3. Update home.tsx to check for registration fee
let homeFile = '/media/callidus/callidus2/ses/Driver-Mobile-Assets/app/(tabs)/home.tsx';
let homeContent = fs.readFileSync(homeFile, 'utf8');

homeContent = homeContent.replace(
  "const expiry = driver?.subscriptionExpiryDate;",
  `if (!driver?.hasPaidRegistrationFee) {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
        alert('Please pay the one-time registration fee to go online.');
        router.push('/registration-fee' as any);
        return;
      }
...
// 4. Also add the screen to _layout.tsx so it routes correctly
let layoutFile = '/media/callidus/callidus2/ses/Driver-Mobile-Assets/app/_layout.tsx';
let layoutContent = fs.readFileSync(layoutFile, 'utf8');
if (!layoutContent.includes('registration-fee')) {
...
console.log('Registration fee flow done');

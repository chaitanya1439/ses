...
const pushTokenRegistry = new Map<string, string>();
 ...
 */
export function registerPushToken(userId: string, pushToken: string): boolean {
 ...
 */
export function unregisterPushToken(userId: string): void {
 ...
 */
export function getAllPushTokens(): Map<string, string> {
...
export interface RideAcceptedNotification {
  ...
  estimatedArrival?: number | undefined; // in minutes
}
...
export async function sendBatchNotifications(
  ...
  const results = new Map<string, ExpoPushTicket | null>();

  const messages: ExpoPushMessage[] = [];
  const userIdOrder: string[] = [];
 ...
 */
export async function notifyDriverOfRideRequest(
  driverId: string,
  rideInfo: RideRequestNotification,
): Promise<ExpoPushTicket | null> {
 ...
 */
export async function notifyRiderOfAcceptance(
  riderId: string,
  acceptInfo: RideAcceptedNotification,
): Promise<ExpoPushTicket | null> {
...
export async function notifyTripStatusChange(
  userId: string,
  statusInfo: TripStatusNotification,
): Promise<ExpoPushTicket | null> {
